...
import { Home, Briefcase, Plane, Plus, ChevronRight, LayoutDashboard, History, StickyNote, Wallet, Settings } from 'lucide-react'; // Added Settings
    ...
    <>
      <div className="w-20 h-full relative">
        <div
          ...
          className="sticky top-24 ml-2 flex flex-col py-3 px-2 rounded-2xl bg-[#0f172a] border border-white/10 shadow-2xl transition-all duration-300 ease-in-out w-20 hover:w-64 h-fit max-h-[82vh] z-[100] group"
        >
                ...
                <div key={wallet.id} className="flex flex-col flex-shrink-0 w-full">
                  <div className="relative w-full group/item"> {/* Added group/item for inner hover */}
                    ...
                    >
                      ...
                      <span className={`whitespace-nowrap font-medium text-sm transition-all duration-300 ${isActive ? 'text-white' : 'text-gray-400'} opacity-0 group-hover:opacity-100 group-hover:translate-x-0 -translate-x-4`}>
                        {wallet.label}
                      </span>
                      ...
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden w-full">
                        <div className="flex flex-col mt-1 ml-9 pl-3 border-l border-white/10 space-y-1 pb-1">
                          {subMenus.map((sub) => {
                            ...
                            return (
                              <Link key={sub.id} to={`${wallet.path}?view=${sub.id}`} className={`flex items-center gap-3 px-3 py-2 rounded-lg text-xs transition-all duration-200 ${isSubActive ? `${wallet.color} bg-white/5 font-medium` : 'text-slate-500 hover:text-slate-300 hover:bg-white/5'}`}>
                                <sub.icon size={14} />
          ...
          <button onClick={() => { setEditingWallet(null); setIsModalOpen(true); }} className="flex-shrink-0 flex items-center gap-4 p-2.5 rounded-xl border border-dashed border-white/10 hover:border-emerald-500/50 hover:bg-emerald-500/10 hover:text-emerald-400 transition-all opacity-60 hover:opacity-100 text-slate-400">
            ...
            <span className="whitespace-nowrap font-medium text-sm opacity-0 group-hover:opacity-100 group-hover:translate-x-0 -translate-x-4 transition-all duration-300">New Wallet</span>
          </button>
        </div>
      </div>
