...
const AddTransactionModal = ({ isOpen, onClose, onAdd }) => {
  ...
  const handleSubmit = (e) => {
    ...
    onAdd({
      ...
      description: description || currentMode.label,
  ...
  return ReactDOM.createPortal(
    <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      ...
      >
        ...
        {/* TABS */}
        {/* FIX APPLIED HERE: Added 'pr-12' to create space on the right for the close button */}
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <motion.div 
                    layoutId="active-tab"
                    className={`absolute bottom-0 left-0 right-0 h-0.5 ${mode.bg.replace('/10', '')}`} 
                  />
                )}
              </button>
            );
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <button 
            ...
            className={`w-full py-3 rounded-xl font-bold text-white shadow-lg transition-all transform active:scale-95 ${
              !amount ? 'opacity-50 cursor-not-allowed bg-slate-700' : `${currentMode.bg.replace('/10', '')} hover:brightness-110`
            }`}
          >
            Add {currentMode.label}
          </button>
